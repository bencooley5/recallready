...
def bar_chart(
    ...
    """Render a restrained categorical chart for an allowlisted count metric."""
...
def heatmap_chart(rows: list[dict[str, object]]) -> alt.Chart:
    """Show recurring derived hazard/product combinations at product-record grain."""
    frame = pd.DataFrame(rows)
    if frame.empty:
        return alt.Chart(
            pd.DataFrame(
                {"primary_hazard": [], "derived_product_category": [], "product_record_count": []}
            )
        ).mark_rect()
    return (
